...
static std::map<int, unsigned int> mapStakeModifierCheckpoints =
    boost::assign::map_list_of(1, 0x75a026ecu)
    (100, 0x6a2079c8u)
    (150, 0x1e2b9d3fu)
    (200, 0x66b6af23u)
    (500, 0xb3abb2c3u);
...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake)
{
    ...
    if (tx.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CZRldStake(spend));
    } else {
        ...
        CRldStake* rldInput = new CRldStake();
        rldInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(rldInput);
    }

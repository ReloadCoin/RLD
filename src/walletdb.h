...
public:
    ...
    bool WriteZRLDSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZRLDSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZRLDSeed_deprecated(uint256& seed);
    bool EraseZRLDSeed();
    bool EraseZRLDSeed_deprecated();

    bool WriteZRLDCount(const uint32_t& nCount);
    bool ReadZRLDCount(uint32_t& nCount);

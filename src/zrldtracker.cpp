...
CzRLDTracker::CzRLDTracker(std::string strWalletFile)
{
...
CzRLDTracker::~CzRLDTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzRLDTracker::Init()
{
...
bool CzRLDTracker::Archive(CMintMeta& meta)
{
...
bool CzRLDTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzRLDTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzRLDTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzRLDTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzRLDTracker::GetSerialHashes()
{
...
CAmount CzRLDTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzRLDTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzRLDTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzRLDTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzRLDTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzRLDTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzRLDTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzRLDTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzRLDTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzRLDTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzRLDTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzRLDWallet* zRLDWallet)
{
    bool iszRLDWalletInitialized = (NULL != zRLDWallet);
    CMintMeta meta;
    ...
    if (! iszRLDWalletInitialized)
        zRLDWallet = new CzRLDWallet(strWalletFile);
    meta.isSeedCorrect = zRLDWallet->CheckSeed(dMint);
    if (! iszRLDWalletInitialized)
        delete zRLDWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzRLDTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzRLDTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzRLDTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzRLDTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzRLDTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzRLDTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzRLDWallet* zRLDWallet = new CzRLDWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            Add(dMint, false, false, zRLDWallet);
        }
        delete zRLDWallet;
        LogPrint("zero", "%s: added %d dzrld from DB\n", __func__, listDeterministicDB.size());
    }

...
CzRLDWallet::CzRLDWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zrld seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZRLDSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZRLDSeed_deprecated()) {
                    LogPrintf("%s: Updated zRLD seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zrld seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zrld wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzRLDWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZRLDCount(nCountLastUsed);
    else if (!walletdb.ReadZRLDCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzRLDWallet::Lock()
{
    seedMaster = 0;
}

void CzRLDWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzRLDWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZRLD(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzRLDWallet::LoadMintPoolFromDB()
{
...
void CzRLDWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzRLDWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzRLDWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zrldTracker->HasPubcoinHash(pMint.first)) {
...
bool CzRLDWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZRLD(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zrldTracker which also adds to database
    pwalletMain->zrldTracker->Add(dMint, true);
...
void CzRLDWallet::SeedToZRLD(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzRLDWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzRLDWallet::GenerateDeterministicZRLD(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzRLDWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzRLDWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzRLDWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());

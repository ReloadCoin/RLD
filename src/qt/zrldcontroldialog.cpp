...
#include "ui_zrldcontroldialog.h"
...
std::set<std::string> ZRldControlDialog::setSelectedMints;
std::set<CMintMeta> ZRldControlDialog::setMints;

bool CZRldControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZRldControlDialog::COLUMN_DENOMINATION || column == ZRldControlDialog::COLUMN_VERSION || column == ZRldControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZRldControlDialog::ZRldControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZRldControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZRldControlDialog::~ZRldControlDialog()
{
    delete ui;
}

void ZRldControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZRldControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZRldControlWidgetItem* itemDenom(new CZRldControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZRldControlWidgetItem *itemMint = new CZRldControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "The zRLD seed used to mint this zRLD is not the same as currently hold in the wallet";
            else
...
void ZRldControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZRldControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZRld_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZRldControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZRldControlDialog::GetSelectedMints()
{
...
void ZRldControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);

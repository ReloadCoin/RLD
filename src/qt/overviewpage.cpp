...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::RLD)
    {
    }
...
void OverviewPage::getPercentage(CAmount nUnlockedBalance, CAmount nZerocoinBalance, QString& sRLDPercentage, QString& szRLDPercentage)
{
    int nPrecision = 2;
    ...
    szRLDPercentage = "(" + QLocale(QLocale::system()).toString(dzPercentage, 'f', nPrecision) + " %)";
    sRLDPercentage = "(" + QLocale(QLocale::system()).toString(dPercentage, 'f', nPrecision) + " %)";
                              ...
                              const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    CAmount rldAvailableBalance = balance - immatureBalance - nLockedBalance;
    CAmount nUnlockedBalance = nTotalBalance - nLockedBalance;

    // RLD Watch-Only Balance
    ...
    // Combined balances
    CAmount availableTotalBalance = rldAvailableBalance + matureZerocoinBalance;
    CAmount sumTotalBalance = nTotalBalance + zerocoinBalance;

    // RLD labels
    ui->labelBalance->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, rldAvailableBalance, false, BitcoinUnits::separatorAlways));
    ...
    // Percentage labels
    ui->labelRLDPercent->setText(sPercentage);
    ui->labelzRLDPercent->setText(szPercentage);

    // Adjust bubble-help according to AutoMint settings
    QString automintHelp = tr("Current percentage of zRLD.\nIf AutoMint is enabled this percentage will settle around the configured AutoMint percentage (default = 10%).\n");
    ...
    if (fEnableZeromint) {
        ...
        automintHelp += tr("To disable AutoMint add 'enablezeromint=0' in rld.conf.");
    }
    else {
        automintHelp += tr("AutoMint is currently disabled.\nTo enable AutoMint change 'enablezeromint=0' to 'enablezeromint=1' in rld.conf");
    }
    ...
    // RLD Available
    bool showRLDAvailable = settingShowAllBalances || rldAvailableBalance != nTotalBalance;
    bool showWatchOnlyRLDAvailable = showRLDAvailable || nAvailableWatchBalance != nTotalWatchBalance;
    ui->labelBalanceText->setVisible(showRLDAvailable || showWatchOnlyRLDAvailable);
    ui->labelBalance->setVisible(showRLDAvailable || showWatchOnlyRLDAvailable);
    ui->labelWatchAvailable->setVisible(showRLDAvailable && showWatchOnly);

    // RLD Pending
    bool showRLDPending = settingShowAllBalances || unconfirmedBalance != 0;
    bool showWatchOnlyRLDPending = showRLDPending || watchUnconfBalance != 0;
    ui->labelPendingText->setVisible(showRLDPending || showWatchOnlyRLDPending);
    ui->labelUnconfirmed->setVisible(showRLDPending || showWatchOnlyRLDPending);
    ui->labelWatchPending->setVisible(showRLDPending && showWatchOnly);
    ...
    // RLD Locked
    bool showRLDLocked = settingShowAllBalances || nLockedBalance != 0;
    bool showWatchOnlyRLDLocked = showRLDLocked || nWatchOnlyLockedBalance != 0;
    ui->labelLockedBalanceText->setVisible(showRLDLocked || showWatchOnlyRLDLocked);
    ui->labelLockedBalance->setVisible(showRLDLocked || showWatchOnlyRLDLocked);
    ui->labelWatchLocked->setVisible(showRLDLocked && showWatchOnly);

    // zRLD
    bool showzRLDAvailable = settingShowAllBalances || zerocoinBalance != matureZerocoinBalance;
    bool showzRLDUnconfirmed = settingShowAllBalances || unconfirmedZerocoinBalance != 0;
    bool showzRLDImmature = settingShowAllBalances || immatureZerocoinBalance != 0;
    ui->labelzBalanceMature->setVisible(showzRLDAvailable);
    ui->labelzBalanceMatureText->setVisible(showzRLDAvailable);
    ui->labelzBalanceUnconfirmed->setVisible(showzRLDUnconfirmed);
    ui->labelzBalanceUnconfirmedText->setVisible(showzRLDUnconfirmed);
    ui->labelzBalanceImmature->setVisible(showzRLDImmature);
    ui->labelzBalanceImmatureText->setVisible(showzRLDImmature);

    // Percent split
    bool showPercentages = ! (zerocoinBalance == 0 && nTotalBalance == 0);
    ui->labelRLDPercent->setVisible(showPercentages);
    ui->labelzRLDPercent->setVisible(showPercentages);
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("RLD")
    updateDisplayUnit();
}

...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::RLD);
    ...
    if (!settings.contains("nAnonymizeRldAmount"))
        settings.setValue("nAnonymizeRldAmount", 1000);

    nAnonymizeRldAmount = settings.value("nAnonymizeRldAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeRldAmount"))
        SoftSetArg("-anonymizerldamount", settings.value("nAnonymizeRldAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in rld.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeRldAmount:
            return QVariant(nAnonymizeRldAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case HideOrphans:
            ...
            break;
        case AnonymizeRldAmount:
            nAnonymizeRldAmount = value.toInt();
            settings.setValue("nAnonymizeRldAmount", nAnonymizeRldAmount);
            emit anonymizeRldAmountChanged(nAnonymizeRldAmount);
            break;
        case CoinControlFeatures:
